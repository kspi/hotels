...
$(function() {
  ...
  var test = function(item, model) {
      ...
      if (model.hall) {
        ...
        if ((item.prep.hall.minCap < model.hall.capacity[0] && item.prep.hall.minCap < model.hall.capacity[0]) || (item.prep.hall.minCap > model.hall.capacity[1] && item.prep.hall.minCap > model.hall.capacity[1])) {
    ...
    setTimeout(function() {
      ...
      if (model.type == 'hall' || model.type == 'hall_hotel') {
        model.hall = {
          ...
          configuration: $.map($sidebar.find('.model-hall-configuration').select2("data"), function(el) {
            return el.id;
          }),
          celebration: $sidebar.find('.model-hall-celebration').is(':checked'),
          conference: $sidebar.find('.model-hall-conference').is(':checked'),
          hardware: $.map($sidebar.find('.model-hall-hardware').select2("data"), function(el) {
            return el.id;
          })
        };
      ...
      if (model.type == 'hotel' || model.type == 'hall_hotel') {
        model.hotel = {
          ...
          food: $.map($sidebar.find('.model-hotel-food').select2("data"), function(el) {
            return el.id;
          }),
